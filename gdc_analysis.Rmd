...
binary_variables <- function(cancer_data, gene_group){
  ...
  final_data <- cbind(MATH_data, variables)[, -c(1)] # The first column is removed since the tumor sample codes are not needed.
  return(final_data)
}
```
...
for (i in 1:length(SNV_pathways)){
  cancer_data <- na.omit(SNV_pathways[[i]])
...
Subset selection methods use least squares to fit a linear model that contains a subset of predictors. An alternative would be to fit a model containing all the predictors using a technique that constrains or regularizes the coefficient estimates or that shrinks them towards zero because this would reduce their variance.
Shrinkage methods, such as ridge regression and lasso, are used to this end. In this case, the lasso will be used because unlike ridge regression, it selects variables since lasso can set coefficients equal to zero.
