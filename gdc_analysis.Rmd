...
# Initial functions
## MATH, binary variables and deleting silent mutations
...
nrows <- nrow(test) # Total rows
two_thirds <- (nrows*2)%/%3 # About 2/3 of the data, rounded down
ncols <- ncol(test)-1 # Number of predictors
regfit.fwd = regsubsets(OUTPUT~., data=test, nvmax=ncols, method = "forward")
plot(regfit.fwd,scale="Cp")

# Model selection using a validation set

set.seed(1)
train = sample(seq(nrows),two_thirds,replace=F) # 2/3 of the data for training the model
regfit.fwd=regsubsets(OUTPUT~.,data=test[train,],nvmax = ncols,method = "forward")

# Predictions on observations not used for training
val.errors=rep(NA,ncols) # ncols: number of variables so, number of subset models
...
plot(sqrt(val.errors),ylab="Root MSE",ylim = c(15,18),pch=19,type="b") # Validation error
points(sqrt(regfit.fwd$rss[-1]/two_thirds),col="blue",pch=19,type = "b") # Root mean Residual sum of squares
