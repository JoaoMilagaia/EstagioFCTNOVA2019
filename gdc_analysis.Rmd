...
The first thing to do is get the data into the environment. Since the data is in several folders in the "SNV" folder, I'm using the "MANIFEST.txt" to get all the file paths. This file also includes the annotations file paths but I'm only interested in the data tables (".maf.gz" files).
...
```
```{r cancer names}
# We do the following to get the cancer names:
cancer_names <- unname(sapply(sapply(paths,strsplit,"[.]"),"[[",2))
                            # Splitting each path by "."
                    # Getting the second element of each split outcome
              # Unname for simplicity sake

cancer_names # Resulting in:
```

Having the file paths and cancer names we can finnaly create a list of data frames, each for each cancer.
...
Since we are studying Intra Tumoral Heterogeneity, we'll have to calculate a unit of it, MATH. For this I'll create a function that takes the data of one cancer and returns a data frame with the calculated MATH for each tumor sample.

To calculate the MATH value of each tumor, one must calculate the MAF values for all the mutations in the tumor, like so:
$$MAF=\frac{t_alt_count}{t_depth}$$

Next, we calculate the MAD value of the tumor:
$$MAD=\frac{1.4826}{n} \sum_{i=1}^{n}\left( \left\lvert{ MAF_{i}-\frac{1}{n} \sum_{i=1}^{n}(MAF_{i}) }\right\rvert \right)$$
And finally, the MATH value:
...
MATH_calculator <- function(interest_data){ # interest_data is the data.frame read from the raw text file
  ...
  MATH_data <-aggregate(interest_data$MAF,                  # Grouping the MAF values
                  list(interest_data$Tumor_Sample_Barcode), # per tumor sample
                  function(MAF){                            # and calculating each tumor's MATH
